const defaultData = {
  ...
  curriculum: { learningArea: "Music", yearLevel: "Year 8", contentCode: "", achievementStandard: "" },
  classes: [
...
const titles = {
  dashboard: ["Command Centre", "One sentence in. Sorted action out."],
...
function loadData() {
  const saved = localStorage.getItem("teacherAssistantDataV14");
  if (!saved) {
    localStorage.setItem("teacherAssistantDataV14", JSON.stringify(defaultData));
  ...
  try {
    ...
    return {
      ...
      curriculum: { ...defaultData.curriculum, ...(parsed.curriculum || {}) },
...
function saveData() {
  localStorage.setItem("teacherAssistantDataV14", JSON.stringify(data));
  renderAll();
}
...
function addTask(title, area = "Admin", priority = "Medium", due = "") {
  data.tasks.unshift({ id: crypto.randomUUID(), title, area, priority, due, complete: false });
}
...
function parseNameFromCommand(command) {
  ...
  const stopWords = ["remind","me","to","email","call","contact","mother","father","parent","carer","add","note","for","attach","photo","assessment","portfolio","create","relief","lesson","rubric","make","draft","year","music","english","maths","science","the"];
  const candidate = words.find(word => /^[A-Z][a-z]+/.test(word) && !stopWords.includes(word.toLowerCase()));
  return candidate || "";
}

function createAIPrompt(taskType, context) {
  const map = {
    lesson: "Create a complete lesson plan",
    relief: "Create a low-prep relief lesson",
    rubric: "Create a student-friendly assessment rubric",
    parent: "Draft a professional parent/carer email",
    report: "Draft report comments",
    differentiate: "Differentiate this learning task"
  };

  promptType.value = taskType === "rubric" ? "rubric" : taskType === "relief" ? "relief" : taskType === "parent" ? "parent" : "lesson";
  promptContext.value = context;
  promptClass.value = data.curriculum.yearLevel + " " + data.curriculum.learningArea;
  promptConstraints.value = "Use Australian school language. Align to Australian Curriculum V9 structure. Keep it practical, quick to use, and realistic for a busy classroom.";
  switchView("ai");

  return `${map[taskType] || "Create teacher support"} prompt prepared in AI Assistant.`;
}
...
function processCommand(command) {
  ...
  if (lower.includes("remind me to") || lower.startsWith("remind")) {
    ...
    const area = lower.includes("email") || lower.includes("mother") || lower.includes("father") || lower.includes("parent") ? "Parent contact" : "Admin";
    addTask(title || text, area, "Medium");
    saveData();
    return `Created task:\n${title || text}\n\nArea: ${area}\nPriority: Medium`;
  }

  if (lower.includes("add note") || lower.includes("note for")) {
    const name = parseNameFromCommand(text);
    const student = findStudentByName(name);
    const note = text.replace(/add note for/i, "").replace(/note for/i, "").replace(name, "").replace(/^[:\s]+/, "").trim();
    if (student) {
      student.portfolio.unshift({ id: crypto.randomUUID(), title: "Teacher note", note: note || text, image: "", date: new Date().toLocaleString() });
      saveData();
      return `Added note to ${student.name}'s portfolio.`;
    }
    addTask(`Create student note: ${text}`, "Portfolio", "Medium");
    saveData();
    return "Student not found. Created a portfolio task instead.";
  }

  if (lower.includes("attach photo")) {
    const name = parseNameFromCommand(text);
    const student = findStudentByName(name);
    if (student) {
      portfolioStudentSelect.value = student.id;
      portfolioTitle.value = "Assessment portfolio photo";
      portfolioNote.value = `Voice/text instruction: ${text}`;
      switchView("students");
      return `Ready to attach a photo to ${student.name}.\nChoose or take a photo, then tap Attach to portfolio.`;
    }
    addTask(`Attach photo to student portfolio: ${text}`, "Portfolio", "High");
    saveData();
    return "Student not found. Created a high-priority portfolio task.";
  }

  if (lower.includes("rubric")) {
    addTask(text, "Assessment", "Medium");
    saveData();
    return createAIPrompt("rubric", text);
  }

  if (lower.includes("relief lesson")) {
    addTask(text, "Planning", "High");
    saveData();
    return createAIPrompt("relief", text);
  }

  if (lower.includes("lesson")) {
    addTask(text, "Planning", "Medium");
    saveData();
    return createAIPrompt("lesson", text);
  }

  if (lower.includes("email") || lower.includes("parent")) {
    addTask(text, "Parent contact", "Medium");
    saveData();
    return createAIPrompt("parent", text);
  }

  addTask(text, "Admin", "Medium");
  saveData();
  return `Command captured as a general task:\n${text}`;
}

processCommandBtn.addEventListener("click", () => {
  commandResult.textContent = processCommand(commandInput.value);
});

clearCommandBtn.addEventListener("click", () => {
  commandInput.value = "";
  commandResult.textContent = "Ready.";
});

document.querySelectorAll(".command-example").forEach(btn => {
  btn.addEventListener("click", () => commandInput.value = btn.dataset.command);
});
...
function setupSpeechRecognition() {
  ...
  recognition.onend = () => voiceStatus.textContent = "Voice idle";
  recognition.onerror = () => voiceStatus.textContent = "Voice error";
  recognition.onresult = event => {
    const transcript = event.results[0][0].transcript;
    commandInput.value = transcript;
    commandResult.textContent = processCommand(transcript);
  };
}
...
lessonForm.addEventListener("submit", e => {
  e.preventDefault();
  data.lessons.unshift({ id: crypto.randomUUID(), day: lessonDay.value, className: lessonClass.value.trim(), topic: lessonTopic.value.trim(), goal: lessonGoal.value.trim(), status: lessonStatus.value });
  e.target.reset();
  saveData();
});
...
classForm.addEventListener("submit", e => {
  e.preventDefault();
  data.classes.unshift({ id: crypto.randomUUID(), name: className.value.trim(), room: classRoom.value.trim(), focus: classFocus.value.trim(), notes: classNotes.value.trim() });
  e.target.reset();
  saveData();
});
...
studentForm.addEventListener("submit", e => {
  e.preventDefault();
  data.students.unshift({ id: crypto.randomUUID(), name: studentName.value.trim(), className: studentClass.value.trim(), notes: studentNotes.value.trim(), portfolio: [] });
  e.target.reset();
  saveData();
});
...
addPortfolioBtn.addEventListener("click", () => {
  ...
  if (file) {
    ...
    reader.onload = () => {
      ...
      student.portfolio.unshift(entry);
...
assessmentForm.addEventListener("submit", e => {
  e.preventDefault();
  data.assessments.unshift({ id: crypto.randomUUID(), className: assessmentClass.value.trim(), title: assessmentTitle.value.trim(), due: assessmentDue.value, status: assessmentStatus.value });
  e.target.reset();
  saveData();
});
...
resourceForm.addEventListener("submit", e => {
  e.preventDefault();
  data.resources.unshift({ id: crypto.randomUUID(), title: resourceTitle.value.trim(), subject: resourceSubject.value.trim(), link: resourceLink.value.trim(), type: resourceType.value });
  e.target.reset();
  saveData();
});
...
function saveCurriculumFromFields() {
  data.curriculum = {
    learningArea: learningArea.value,
    yearLevel: yearLevel.value,
    contentCode: contentCode.value.trim(),
    achievementStandard: achievementStandard.value.trim()
  };
  localStorage.setItem("teacherAssistantDataV14", JSON.stringify(data));
}

["change", "input"].forEach(evt => {
  [learningArea, yearLevel, contentCode, achievementStandard].forEach(el => el.addEventListener(evt, () => {
    saveCurriculumFromFields();
    renderDashboard();
  }));
});
...
promptForm.addEventListener("submit", e => {
  ...
  const taskMap = {
    lesson: "Create a complete lesson plan",
    relief: "Create a relief lesson plan",
    differentiate: "Differentiate this learning task",
  ...
  promptOutput.textContent = `${taskMap[promptType.value]} for ${promptClass.value || data.curriculum.yearLevel + " " + data.curriculum.learningArea}.

Teacher mode:
${data.settings.mainMode}

Australian Curriculum V9 structure:
Learning area: ${data.curriculum.learningArea}
Year level/band: ${data.curriculum.yearLevel}
Content code: ${data.curriculum.contentCode || "Not supplied"}
Achievement standard focus: ${data.curriculum.achievementStandard || "Not supplied"}
...
- timing
- success criteria
- assessment/evidence suggestion
- Australian school language
- low-admin teacher workflow`;
});
...
function renderDashboard() {
  ...
  welcomeTitle.textContent = `${data.settings.teacherName || "Teacher"}, tell me what needs doing.`;

  priorityStack.innerHTML = openTasks.length
    ? openTasks.slice(0, 6).map(t => `<li>${escapeHtml(t.priority)} priority: ${escapeHtml(t.title)}</li>`).join("")
    : "<li>No open tasks yet.</li>";

  curriculumSummaryTitle.textContent = `${data.curriculum.yearLevel} ${data.curriculum.learningArea}`;
  curriculumSummaryText.textContent = `Content code: ${data.curriculum.contentCode || "not set"} · Achievement standard: ${data.curriculum.achievementStandard || "not set"}`;
}
...
function renderCurriculumFields() {
  learningArea.value = data.curriculum.learningArea || "Music";
  yearLevel.value = data.curriculum.yearLevel || "Year 8";
  contentCode.value = data.curriculum.contentCode || "";
  achievementStandard.value = data.curriculum.achievementStandard || "";
}
